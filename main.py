...
# 2. 국내 한정 뉴스 검색 및 파이썬을 이용한 '엄격한 3개월 날짜 필터링'
query = "국내 (인수합병 OR 지분투자 OR 매각 OR 경영권) (반도체 OR 바이오 OR 제약 OR 헬스케어 OR 배터리 OR 2차전지) when:3m"
...
if not news_context.strip():
    ...
    [엄격한 필수 조건]
    1. **절대 전체 요약, 인사말, 시장 총평 등을 작성하지 마세요.** 오직 아래 [출력 형식]의 HTML <div> 태그 블록만 출력해야 합니다.
    2. '인수 추진', '매각 검토', '투자 유치' 등 진행 중인 사안도 포함하세요.
    3. 동일한 M&A/투자 건에 대해 여러 개의 관련 기사가 있다면, 하나의 카드로 묶고 '기사 원문' 항목에 최대 3개까지 링크를 나열하세요. (예: <a href="링크1">기사 1</a> <a href="링크2">기사 2</a>)
    4. 발견된 딜이 어느 분야인지 판단하여 '반도체', '바이오', '배터리', '기타' 중 1개로 분류하세요.
    5. 발견된 딜 개수만큼 [출력 형식]을 반복해서 모두 출력하세요.
    ...
    # AI가 혹시라도 마크다운 코드 블록(```html)을 씌울 경우를 대비해 제거
    deal_content = result.text.replace('```html', '').replace('```', '').strip()

# 4. 필터 버튼 및 자바스크립트가 포함된 HTML 생성
